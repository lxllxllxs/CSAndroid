 ...
 */
public class CancleOrderDetailAty extends BaseActivity {
    ...
    private void initView() {
        ...
        edtReason.setText(order.getReason());

        //用退货订单的状态
        setConfiromButton(Integer.valueOf(order.getStatus()));
    ...
    private void setAllUnEditable() {
        ...
        edtAddress.setFocusable(false);
        edtReason.setFocusable(false);
    }

    CancelOrder order;
    private void initData() {
        order= getIntent().getParcelableExtra("cancelOrder");
        pOrderId = Integer.valueOf(order.getOrderId());
    ...
    private void updateProductOrderState() {
        Map<String, Object> params = new HashMap<>();
        params.put("poId", pOrderId);
        params.put("cancelOrderId", String.valueOf(order.getId()));//这里用String
        ShopService service = RetrofitUtil.create(ShopService.class);
        showLoadDialog("");
        Call<ResponseBody> call = service.updateCancelOrderState(params);
        call.enqueue(new Callback<ResponseBody>() {
            @Override
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                dismissDialog();
    ...
    /**
     * 状态1为 退货状态订单激活 等待退货操作
     * @param type
     */
    private void setConfiromButton(int type) {
        switch (type) {
            case 0:
                tvConfirm.setText("已退货");
                tvConfirm.setBackgroundColor(ContextCompat.getColor(this, R.color.gray_text));
                break;
            case 1:
                tvConfirm.setText("同意退货");
                tvConfirm.setBackgroundColor(ContextCompat.getColor(this, R.color.theme_red));
                break;
        }
    }
}
